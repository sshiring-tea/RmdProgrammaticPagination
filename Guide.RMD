...
- For Word and PDF documents, use `\pagebreak` (alternatively, `\newpage` also works)
...
To complicate matters, `\pagebreak` will produce a new page/slide in an HTML 
presentation, but *only* when used outside of an R code block. `\pagebreak` will
not work if used programmatically from inside an R code block. You can use both
`\pagebreak` and `---` inside a single HTML presentation Markdown document without
